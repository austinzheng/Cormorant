...
class TestTestHelpers : InterpreterTest {
  ...
  func testListForItems() {
    let ref : ConsValue = .List(Cons(.IntAtom(1),
      next: Cons(.StringAtom("foo"),
        next: Cons(.BoolAtom(true)))))
    let list = listWithItems(.IntAtom(1), .StringAtom("foo"), .BoolAtom(true))
  ...
  func testExpectThat() {
    ...
    expectThat("a", shouldEvalTo: listWithItems(.IntAtom(1), .IntAtom(2), .IntAtom(3),
      .IntAtom(4), .IntAtom(5)))
    let value = runCode("a")
    let refList = listWithItems(.IntAtom(1), .IntAtom(2), .IntAtom(3), .IntAtom(4),
      .IntAtom(5))
