...
class TestDissocBuiltin : InterpreterTest {
  ...
  func testEchoingArg() {
    expectThat("(.dissoc nil)", shouldEvalTo: .Nil)
    expectThat("(.dissoc 1529)", shouldEvalTo: 1529)
    expectThat("(.dissoc true)", shouldEvalTo: true)
    expectThat("(.dissoc \"foobar\")", shouldEvalTo: .StringAtom("foobar"))
    expectThat("(.dissoc '(1 2 3))", shouldEvalTo: listWithItems(1, 2, 3))
    expectThat("(.dissoc [1 2 3])", shouldEvalTo: vectorWithItems(1, 2, 3))
    expectThat("(.dissoc {1 2 3 4})", shouldEvalTo: mapWithItems((1, 2), (3, 4)))
  }

  /// .dissoc should return nil if called with keys on nil.
  func testWithNil() {
    expectThat("(.dissoc nil 0)", shouldEvalTo: .Nil)
    expectThat("(.dissoc nil \"foo\" :bar 'baz)", shouldEvalTo: .Nil)
  }
  ...
  func testWithValidKeys() {
    expectThat("(.dissoc {1 \"one\" 2 \"two\" 3 \"three\"} 1)",
      shouldEvalTo: mapWithItems((2, .StringAtom("two")), (3, .StringAtom("three"))))
    expectThat("(.dissoc {1 \"one\" 2 \"two\" 3 \"three\"} 2)",
      shouldEvalTo: mapWithItems((1, .StringAtom("one")), (3, .StringAtom("three"))))
    expectThat("(.dissoc {1 \"one\" 2 \"two\" 3 \"three\"} 1 3)",
      shouldEvalTo: mapWithItems((2, .StringAtom("two"))))
  ...
  func testWithInvalidKeys() {
    expectThat("(.dissoc {1 \"one\" 2 \"two\" 3 \"three\"} 4)",
      shouldEvalTo: mapWithItems((1, .StringAtom("one")), (2, .StringAtom("two")), (3, .StringAtom("three"))))
    expectThat("(.dissoc {1 \"one\" 2 \"two\" 3 \"three\"} \"1\" :2 \\3)",
      shouldEvalTo: mapWithItems((1, .StringAtom("one")), (2, .StringAtom("two")), (3, .StringAtom("three"))))
    expectThat("(.dissoc {} nil)", shouldEvalTo: mapWithItems())
  }
