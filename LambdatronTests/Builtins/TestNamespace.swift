...
class TestNamespaceBuiltin : InterpreterTest {
  ...
  func testUnqualifiedSymbols() {
    expectThat("(.namespace 'a)", shouldEvalTo: .Nil)
    expectThat("(.namespace 'fooooooooooo)", shouldEvalTo: .Nil)
  }

  /// .namespace should return the namespace as a string for qualified symbols.
  func testQualifiedSymbols() {
    expectThat("(.namespace 'user/a)", shouldEvalTo: .StringAtom("user"))
    expectThat("(.namespace 'foo/bar/baz)", shouldEvalTo: .StringAtom("foo"))
  }
