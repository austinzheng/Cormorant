...
class TestFnPositionVectors : InterpreterTest {
  ...
  func testWithSymbol() {
    ...
    expectThat("(testVec 2)", shouldEvalTo: .IntAtom(9))
  }
...
class TestFnPositionMaps : InterpreterTest {
  ...
  func testValidKey() {
    expectThat("({:a 1 :b 2 :c 3} :a)", shouldEvalTo: .IntAtom(1))
    expectThat("({:a 1 :b 2 :c 3} :b)", shouldEvalTo: .IntAtom(2))
    expectThat("({:a 1 :b 2 :c 3} :c)", shouldEvalTo: .IntAtom(3))
  }
  ...
  func testValidKeyWithFallback() {
    expectThat("({:a 1 :b 2 :c 3} :a 501)", shouldEvalTo: .IntAtom(1))
    expectThat("({:a 1 :b 2 :c 3} :b 230)", shouldEvalTo: .IntAtom(2))
    expectThat("({:a 1 :b 2 :c 3} :c 101)", shouldEvalTo: .IntAtom(3))
  }
...
class TestFnPositionSymbols : InterpreterTest {
  ...
  func testValidKey() {
    expectThat("('a {'a 1 'b 2 'c 3})", shouldEvalTo: .IntAtom(1))
    expectThat("('b {'a 1 'b 2 'c 3})", shouldEvalTo: .IntAtom(2))
    expectThat("('c {'a 1 'b 2 'c 3})", shouldEvalTo: .IntAtom(3))
  }

  /// A symbol in function position should return values for valid keys, ignoring the fallback argument.
  func testValidKeyWithFallback() {
    expectThat("('a {'a 1 'b 2 'c 3} 501)", shouldEvalTo: .IntAtom(1))
    expectThat("('b {'a 1 'b 2 'c 3} 230)", shouldEvalTo: .IntAtom(2))
    expectThat("('c {'a 1 'b 2 'c 3} 101)", shouldEvalTo: .IntAtom(3))
  }
  ...
  func testWithUnsupportedFallback() {
    expectThat("('foo nil 150)", shouldEvalTo: .IntAtom(150))
    expectThat("('foo true 88)", shouldEvalTo: .IntAtom(88))
    expectThat("('foo false 512)", shouldEvalTo: .IntAtom(512))
    expectThat("('foo 159 89)", shouldEvalTo: .IntAtom(89))
    expectThat("('foo -2.9981 10001)", shouldEvalTo: .IntAtom(10001))
    expectThat("('foo :a -57)", shouldEvalTo: .IntAtom(-57))
    expectThat("('foo 'a 2)", shouldEvalTo: .IntAtom(2))
    expectThat("('foo \\a 3)", shouldEvalTo: .IntAtom(3))
    expectThat("('foo \"foobar\" 834)", shouldEvalTo: .IntAtom(834))
    expectThat("('foo '('foo 'bar) 141)", shouldEvalTo: .IntAtom(141))
    expectThat("('foo ['foo 'bar] 600000)", shouldEvalTo: .IntAtom(600000))
    expectThat("('foo .+ 15)", shouldEvalTo: .IntAtom(15))
  }
